...
import static pt.up.fe.comp2023.jasmin.MethodSpecsGetter.*;
...
public class Translator {
    ...
    public String translateInstruction(GetFieldInstruction instruction, Method ancestorMethod) {
        ...
        jasminInstruction.append(" ").append(translateType(ancestorMethod.getOllirClass(), destinationField.getType()));
    ...
    public String translateInstruction(CallInstruction instruction, Method ancestorMethod) {
        ...
        switch (callType) {
            case invokestatic, invokevirtual -> {
                instruction.show();
                instruction.getFirstArg().show();
                System.out.println(instruction.getNumOperands());
                ...
                for (Element element : instruction.getListOfOperands()) {
                    ...
                    parametersDescriptor.append(translateType(ancestorMethod.getOllirClass(), element.getType()));
                }
                ...
                if (callType == CallType.invokestatic) {
                    ...
                    jasminInstruction.append(getFullClassName(ancestorMethod.getOllirClass(), classType.getName()));
                }
                jasminInstruction.append(".").append(trim(methodName.getLiteral()));
                jasminInstruction.append("(").append(parametersDescriptor);
                jasminInstruction.append(")").append(translateType(ancestorMethod.getOllirClass(), instruction.getReturnType()));
            }
            case invokespecial -> {
        ...
        System.out.println(jasminInstruction);
                ...
                if (operationType == OperationType.ADD) {
                    ...
                    try {
                        if (second.isLiteral()) {
                            ...
                            int literal = Integer.parseInt(trim(literalElement.getLiteral()));
                            if (literal == 0) {
    ...
    private String getIinc(Method ancestorMethod, LiteralElement literalElement, Operand operand, StringBuilder jasminInstruction) {
        ...
        jasminInstruction.append(trim(literalElement.getLiteral()));
        ...
        if (element.isLiteral()) {
            ...
            switch (literalElement.getType().getTypeOfElement()) {
                case INT32, BOOLEAN -> {
                    ...
                    String literal = trim(literalElement.getLiteral());
                    try {
