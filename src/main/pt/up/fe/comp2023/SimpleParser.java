...
public class SimpleParser implements JmmParser {
    ...
    public JmmParserResult parse(String jmmCode, String startingRule, Map<String, String> config) {
        ...
        try {
            ...
            System.out.println("num Errors:" + parser.getNumberOfSyntaxErrors());
            return JmmParserResult.newError(new Report(ReportType.ERROR, Stage.SYNTATIC, -1,
                    String.format("There were syntax %d errors during parsing, terminating",parser.getNumberOfSyntaxErrors())));
        }
    }
}
